...
Fourier 投影 `C_n(d) = (1/T_0)\int e^{in\Omega_0 t} e^{-ia(t)\cdot d} dt`（`T_0 = 2π/Ω₀`）。
共振条件 `n + Σ_α l_α m_α = 0`，即选择定则 **`Σ_α l_α m_α = −n`**（已由直接数值积分验证；
等价的另一种写法是 `Σ l m = +n` 配相位 `e^{+imδ}`——两者不可混用）：

```math
\boxed{
C_n(d)
=
\sum_{\{m_\alpha\}\,:\,\sum_\alpha l_\alpha m_\alpha = -n}
...
顺序折叠（离散卷积，注意 **`+l_α m`** 的方向来自共振条件 `n + Σ l m = 0`）：

```math
S^{(0)}_n = \delta_{n,0},
\qquad
S^{(\alpha)}_n = \sum_{m=-M_\alpha}^{M_\alpha} S^{(\alpha-1)}_{n + l_\alpha m}\; B_\alpha(m).
```

全部模折叠完成后 `C_n = S^{(N_mode)}_n`。

**复杂度**：`O(N_mode × N_n × M_avg)`，与 `n_time` 无关。
...
记号：`A_R := T_n(R) = t(R)·C_n(d_R)`，`B_R := T_{−n}(R)`（都是 `FloquetHarmonicCache` 的块）。
...
T_0(R)
+
\sum_{n=1}^{n_{\max}} \frac{\mathrm{comm}_n(R)}{n\,\hbar\Omega_0}
}
```

- **support 自动确定**：`supp(T_eff) = supp(T_0) ∪ ⋃_n [supp(A) + supp(B)]`（Minkowski 和），
  不再需要 `target_hamR` 参数；
- **无 k_mesh、无混叠、无逆 FT**；
- 复杂度 `O(|supp|² × harmonic_max × nsta³)`，每个乘积 `A_{R−R'}·B_{R'}`、`B_{R−R'}·A_{R'}` 用 `blas::zgemm`
  （两个卷积都要算，"P − P† 半乘法"不成立）。
...
/// drive: 模集合（l_α, a_α）
/// harmonic range: [harmonic_min, harmonic_max]
/// 返回 C_n(d)，n = harmonic_min..=harmonic_max
...
3. `S` 序列按 §2.1 折叠（长度 `harmonic_max − harmonic_min + 1`，卷积边界 clamp 到范围外=0）
4. 返回 `S`
...
新增：**按 distinct `d` 去重的 `C_n` 缓存**（`HashMap<[u64; DIM] 位模式, Vec<Complex<f64>>>`）——
spin up/down 共用同一 `d`。对称键 `±d` 互查（`C_n(−d) = C_{−n}(d)*`）留作后续优化
（要求 n 范围对称；当前调用点均满足，先保持最小改动）。

### 4.4 实空间对易子

```rust
/// A_R = T_n(R)、B_R = T_{−n}(R)，均与 hamR 一一对应（来自谐波缓存）。
/// 返回 (comm(R) 块序列, 其 R 矢量序列)。
fn real_space_commutator(
    a_blocks: &[Array2<Complex<f64>>],   // T_n(R)，长度 = hamR.nrows()
    b_blocks: &[Array2<Complex<f64>>],   // T_{−n}(R)，长度 = hamR.nrows()
    ham_r: &Array2<isize>,
...
| 多频公度 | 多模 `l_α = p_α` | 选择定则 `Σ l_α m_α = −n` 自动处理 |
...
2. **单模系数**：`C_n = (−i)^n J_n(R)e^{+inδ}`（复振幅情形的正确相位；`δ ∈ {0,π}` 时退化为实形式）显式公式 vs `bessel_peierls_coeffs`
3. **多模混频**：`l=(1,2)`、`l=(1,3)` 的 `C_n` vs 时间网格 DFT（误差 < 1e-10）
4. **四束 tetrahedral**：`C_n` vs DFT；`T_eff` vs legacy k 空间路径（块级比较，误差 < 1e-10）
...
8. **性能 smoke**（已实现）：2D 双轨道模型、harmonic_max=2，warmup 后 Bessel 路径 vs legacy
...
- `R_α > R_max`（如 8）或模数 > 8：自动回退 `TimeGrid`（保持通用性）。回退网格的
  分辨率按 `max(2·Σ_α |l_α|·M_α(R_α) + 4, 2·max(|harmonic_min|,|harmonic_max|) + 1)` 直接确定
...
- Bessel 与 DFT 的 `C_n` 在测试中交叉验证，发布前默认后端以测试通过为准

---

## 8. 实施顺序（每步可提交）

1. `bessel_j` + 单模 `C_n` + 测试 1–2
2. 递归卷积多模 `bessel_peierls_coeffs` + 测试 3
3. d 去重缓存接入 `floquet_harmonic_cache`
4. `real_space_commutator`（两个卷积 `(AB)(R)`、`(BA)(R)`，均 zgemm）+ 单 n 测试
...
`H_n^ours(k) == H_n^lit(−k)` 逐元素（1e-12），覆盖 n ∈ [−harmonic_max, harmonic_max]、多个 k、
...
N 即 harmonic_max 截断；断言 d_z(k) 的 k 依赖与系数随 harmonic_max 增大收敛（n=3m 项严格
