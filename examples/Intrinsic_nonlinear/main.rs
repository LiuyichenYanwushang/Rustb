...
fn main() {
    ...
    */
    ...
    let mu0=-0.04;
    let (omega_one,band,partial_G)=model.berry_connection_dipole_onek(&k,&dir_1,&dir_2,&dir_3,0);
    ...
    let mu=Array1::linspace(E_min,E_max,E_n);
    let conductivity=mu.par_iter().map(|x| {
        let f:Array3::<f64>=1.0/(beta*(&band-*x)).map(|x| x.exp()+1.0);
        let pf=-&f*(1.0-&f)*beta;
        let a=(&berry_curv*&pf).sum_axis(Axis(2)).sum()/(nk.pow(2) as f64)*ratio.powi(2);
        a/PI/2.0}).collect();
    let mut fg = Figure::new();
    let x:Vec<f64>=mu.to_vec();
    let axes=fg.axes2d();
