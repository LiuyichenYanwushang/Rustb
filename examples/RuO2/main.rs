...
use rayon::prelude::*;
use gnuplot::{Figure, Caption, Color,Fix,LineStyle,Solid,Auto};
...
//    println!("{}",conductivity/(2.0*PI));
    ...
    */
    ...
    let result:Vec<f64>=theta.iter().map(|x| {
    ...
    let cond_xy=conductivity_all(&model,&k_mesh,&dir_1,&dir_2,mu,spin,eta);
    println!("{}",cond_xy);
    println!("{}",conductivity_onek(&model,&k,&dir_1,&dir_2,mu,spin,eta));
}
#[inline(always)]
fn conductivity_onek(model:&Model,k_vec:&Array1::<f64>,dir_1:&Array1::<f64>,dir_2:&Array1::<f64>,mu:f64,spin:usize,eta:f64)->f64{
    ...
    for i in 0..model.nsta{
        for j in 0..model.nsta{
            U0[[i,j]]=1.0/(((mu-band[[i]]).powi(2)+eta0)*((mu-band[[j]]).powi(2)+eta0))+0.0*li;
        }
    }
...
fn conductivity_all(model:&Model,k_mesh:&Array1::<usize>,dir_1:&Array1::<f64>,dir_2:&Array1::<f64>,mu:f64,spin:usize,eta:f64)->f64{
    let k_vec=gen_kmesh(&k_mesh);
