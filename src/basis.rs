...
impl Model{
    ...
    pub fn set_hop<T:Data<Elem=isize>,U:hop_use>(&mut self,tmp:U,ind_i:usize,ind_j:usize,R:&ArrayBase<T,Ix1>,pauli:isize){
        ...
        */
        ...
        if R_exist {
            ...
            update_hamiltonian!(self.spin,pauli,tmp,self.ham.slice_mut(s![index,..,..]),ind_i,ind_j,norb);
            if negative_R_exist && ind_i != ind_j{
                update_hamiltonian!(self.spin,pauli,tmp.conj(),self.ham.slice_mut(s![0,..,..]),ind_j,ind_i,norb);
            }
            ...
            update_hamiltonian!(self.spin,pauli,tmp.conj(),self.ham.slice_mut(s![index,..,..]),ind_j,ind_i,norb);
    ...
    pub fn add_hop<T:Data<Elem=isize>,U:hop_use>(&mut self,tmp:U,ind_i:usize,ind_j:usize,R:&ArrayBase<T,Ix1>,pauli:isize){
        ...
        if R_exist {
            let index=index_R(&self.hamR,&R);
            add_hamiltonian!(self.spin,pauli,tmp,self.ham.slice_mut(s![index,..,..]),ind_i,ind_j,norb);
            if negative_R_exist && ind_i !=ind_j{
                add_hamiltonian!(self.spin,pauli,tmp.conj(),self.ham.slice_mut(s![0,..,..]),ind_j,ind_i,norb);
            }
            if ind_i==ind_j && tmp.im !=0.0 && (pauli==0 ||pauli==3) && index==0{
                panic!("Wrong, the onsite hopping must be real, but here is {}",tmp)
            }
            ...
            let index=index_R(&self.hamR,&negative_R);
            add_hamiltonian!(self.spin,pauli,tmp.conj(),self.ham.slice_mut(s![index,..,..]),ind_j,ind_i,norb);
        }else{
            let mut new_ham=Array2::<Complex<f64>>::zeros((self.nsta(),self.nsta()));
    ...
    pub fn cut_piece(&self,num:usize,dir:usize)->Model{
        ...
        for i in 0..num{
            ...
            for n in 0..self.natom(){
                ...
                let use_atom=Atom::new(use_atom_position,self.atoms[n].norb(),self.atoms[n].atom_type());
                new_atom.push(use_atom);
            }
        }
...
pub fn cut_dot(&self,num:usize,shape:usize,dir:Option<Vec<usize>>)->Model{
            ...
            let (old_model,use_orb_item,use_atom_item)={
                ...
                match shape{
                    ...
                    4=>{
                        ...
                        for i in 0..model_2.natom(){
                            let atom_position=model_2.atoms[i].position();
                            if atom_position[[dir[0]]]*(num0+1.0)/num0> 1.0+1e-5 || atom_position[[dir[1]]]*(num0+1.0)/num0> 1.0+1e-5  {
                                a+=model_2.atoms[i].norb();
                            }else{
                                use_atom_item.push(i);
                                for i in 0..model_2.atoms[i].norb(){
            ...
            let (old_model,use_orb_item,use_atom_item)={
                ...
                match shape{
                    ...
                    4=>{
                        ...
                        for i in 0..model_2.natom(){
                            let atom_position=model_2.atoms[i].position();
                            if atom_position[[0]]*(num0+1.0)/num0> 1.0+1e-5 || atom_position[[1]]*(num0+1.0)/num0> 1.0+1e-5  {
                                a+=model_2.atoms[i].norb();
                            }else{
                                use_atom_item.push(i);
                                for i in 0..model_2.atoms[i].norb(){
                    ...
                    6=>{
                        for i in 0..model_2.natom(){
                            let atom_position=model_2.atoms[i].position();
                            if (atom_position[[1]]-atom_position[[0]] + 0.5 < 0.0) || (atom_position[[0]]-atom_position[[1]] + 0.5 < 0.0) {
                                a+=model_2.atoms[i].norb();
                            }else{
                                use_atom_item.push(i);
                                for i in 0..model_2.atoms[i].norb(){
    ...
    //!\\end{aligned}\rt\.$$
        ...
        let unfold_atom=&atom_position.dot(U).map(|x| if (x.fract()-1.0).abs()<precision || x.fract().abs()<precision {0.0}else if x.fract()<0.0 {x.fract()+1.0} else {x.fract()});
        ...
        for (i,u_atom) in unfold_atom.outer_iter().enumerate(){
            ...
            if exist && a!=0{
                ...
                for i0 in unit_atom_orb_match[index]..unit_atom_orb_match[index]+self.atoms[i].norb(){
                    match_orb_list[[b]]=i0;
                    b+=1;
                }
            }else{
