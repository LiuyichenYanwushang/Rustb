pub mod ndarray_lapack{
        ...
        S:Data<Elem=Complex<f64>>,
    {
        ...
        if info == 0 {
            // 将特征值向量转换为一维数组并返回
            (Array1::<f64>::from_vec(w.into_iter().take(m as usize).collect()),Array2::<Complex<f64>>::from_shape_vec([m as usize,n as usize],z.into_iter().take((n*m) as usize).collect()).unwrap())
        } else {
            // 报告错误信息
    ...
    pub fn eigvalsh_x<S>(x: &ArrayBase<S,Ix2>, range: (f64,f64),epsilon:f64,uplo:UPLO) -> Array1<f64>
    where
        S:Data<Elem=Complex<f64>>,
    {
        // 获取矩阵的阶数
    ...
    pub fn eigh_r<S>(x: &ArrayBase<S,Ix2>, range: (f64,f64),epsilon:f64,uplo:UPLO) -> (Array1<f64>,Array2<Complex<f64>>)
    where
        S:Data<Elem=Complex<f64>>,
    {
    ...
    pub fn eigvalsh_r<S>(x: &ArrayBase<S,Ix2>, range: (f64,f64),epsilon:f64,uplo:UPLO) -> Array1<f64>
    where
        S:Data<Elem=Complex<f64>>,
    {
        // 获取矩阵的阶数
