...
use crate::{gen_kmesh,gen_krange,comm,anti_comm,Model};
...
impl Model {
    ...
    #[inline(always)]
    pub fn optical_geometry_n_onek<S:Data<Elem=f64>>(&self,
        k_vec:&ArrayBase<S,Ix1>,
        dir_1:&Array1::<f64>,
        dir_2:&Array1::<f64>,
        og:&Array1<f64>,
        eta:f64) -> (Array2::<Complex<f64>>, Array2::<Complex<f64>>, Array1::<f64>) {
        ...
        let J = J.outer_iter()
            .zip(dir_1.iter())
            .fold(Array2::zeros((self.nsta(), self.nsta())),|acc, (x, d)| {acc + &x * (*d + 0.0 * li)});
        ...
        Zip::from(omega_n.outer_iter_mut())
            ...
            .apply(|mut omega, mut matric, a0| {
                ...
                let o = im.outer_iter().zip(UU.outer_iter()).map(|(a, b)| a.dot(&b)).collect();
                let m = re.outer_iter().zip(U1.outer_iter()).map(|(a, b)| a.dot(&b)).collect();
    ...
    pub fn optical_conductivity_all_direction(&self,k_mesh:&Array1<usize>,T:f64,mu:f64,og:&Array1<f64>,eta:f64)->(Array2::<Complex<f64>>,Array2::<Complex<f64>>)
    {
        let li:Complex<f64>=1.0*Complex::i();
        let kvec:Array2::<f64>=gen_kmesh(k_mesh);
        let nk:usize=kvec.len_of(Axis(0));
        ...
        let (matric_sum,omega_sum)=match self.dim_r(){
            3=>{
                let omega=omega.into_iter().fold(Array2::zeros((6,n_og)), |omega_acc,omega|{ omega_acc+omega });
                let matric=matric.into_iter().fold(Array2::zeros((3,n_og)), |matric_acc,matric|{ matric_acc+matric });
                (omega,matric)
            },
            2=>{
                let omega=omega.into_iter().fold(Array2::zeros((3,n_og)), |omega_acc,omega|{ omega_acc+omega });
                let matric=matric.into_iter().fold(Array2::zeros((1,n_og)), |matric_acc,matric|{ matric_acc+matric });
                (omega,matric)
            },
            _=>panic!("Wrong, self.dim_r must be 2 or 3 for using optical_conductivity_all_direction")
        };
